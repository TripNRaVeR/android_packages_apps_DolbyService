...
public class IeqTranslator implements Translator
{
    static final Parameter[] srcParams;
    
    static {
        srcParams = new Parameter[] { Parameter.ieq_frequency, Parameter.ieq_target };
    }
    ...
    public void dap1Translate(final SetParamCommand setParamCommand, final ParameterValues parameterValues, final Endpoint endpoint) {
        final int[] value = parameterValues.get(Parameter.ieq_frequency);
        if (value == null) {
            return;
        }
        final int[] value2 = parameterValues.get(Parameter.ieq_target);
        setParamCommand.add(DapParameter.DAP1_IENB, value.length);
        setParamCommand.add(DapParameter.DAP1_IEBF, value);
        setParamCommand.add(DapParameter.DAP1_IEBT, value2);
    ...
    public void dap2Translate(final SetParamCommand setParamCommand, final ParameterValues parameterValues, final Endpoint endpoint) {
        ...
        setParamCommand.add(DapParameter.DAP2_IEBS, value.length, new int[][] { value, parameterValues.get(Parameter.ieq_target) });
    }
    
    @Override
    public Parameter[] srcParams() {
        return IeqTranslator.srcParams;
    }
}
