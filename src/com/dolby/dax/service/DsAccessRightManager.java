...
import tripndroid.dolby.audio.api.DsConstants;
import tripndroid.dolby.audio.api.DsAccess;
...
public class DsAccessRightManager
{
    ...
    private int globalAccessHandle_;
    ...
    public int queryDsVersion(int handle) {
        if (dsVersionMap_.containsKey(Integer.valueOf(handle))) {
            return ((Integer) dsVersionMap_.get(Integer.valueOf(handle))).intValue();
        }
        return 0;
    }
     ...
     * @return true The HashMap dsVersionMap_ is empty, else false.
     */
    ...
    /**
     * Change the access right when an application lost or gained audio focus. (legacy)
     */
    public boolean doAccessForLegacyClient(int handle, boolean isSetting) {
        if (queryDsVersion(handle) != 1) {
            return false;
        }
        if (isSetting) {
            for (Map.Entry<Integer, Integer> x : this.dsAccessRightLookupMap_.entrySet()) {
                int key = ((Integer) x.getKey()).intValue();
                int appId = ((Integer) x.getValue()).intValue();
                if (key == 2) {
                    if (!this.cbkManager_.invokeCallback(11, appId, 0, 0, null, null)) {
                        return false;
                    }
                    clearDsAccessRight(appId, 2);
                    globalAccessHandle_ = appId;
                    return true;
                }
            }
            return false;
        }
        setDsAccessRight(globalAccessHandle_, 2);
        globalAccessHandle_ = -1;
        return false;
    }

    public boolean isDsFocusGranted() {
        synchronized (lock_) {
            for (Map.Entry<Integer, Integer> x : this.dsAccessRightLookupMap_.entrySet()) {
                if (((Integer) x.getKey()).intValue() == 1) {
                    return true;
                }
            }
            return false;
